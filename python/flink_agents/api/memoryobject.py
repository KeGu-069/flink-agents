...
# class LocalMemoryObject(MemoryObject):
#     def __init__(self, store: dict, prefix: str = ""):
#         self._store = store   # flatten dict
#         self._prefix = prefix # current object path
#
#     def get(self, path):
#         abs_path = self._full_path(path)
#         if abs_path in self._store:
...
#             partial = ".".join(parts[:i])
#             if partial not in self._store:
#                 self._store[partial] = {"__type__": "__OBJ__", "children": set()}
#             parent = ".".join(parts[:i - 1]) if i > 1 else ""
#             if parent:
#                 self._store[parent]["children"].add(parts[i - 1])
#
#         if len(parts) >= 1:
#             parent = ".".join(parts[:-1])
#             if parent:
#                 self._store[parent]["children"].add(parts[-1])
#
#         if abs_path in self._store and self._is_object_marker(self._store[abs_path]):
#             raise ValueError(f"Cannot overwrite object field '{abs_path}' with primitive.")
#
...
#         self._store[abs_path] = value
#
#     def new_object(self, path: str, overwrite: bool = False) -> MemoryObject:
#         abs_path = self._full_path(path)
#         if abs_path in self._store:
#             if not self._is_object_marker(self._store[abs_path]):
#                 if not overwrite:
#                     raise ValueError(
#                         f"Cannot create object at '{abs_path}': "
#                         "field exists and is not an object. Use overwrite=True to force."
#                     )
#                 else:
#                     # 覆盖掉原来的值
#                     self._store[abs_path] = {"__type__": "__OBJ__", "children": set()}
#         else:
#             self._store[abs_path] = {"__type__": "__OBJ__", "children": set()}
#
#         # 自动补充中间路径
...
#     def get_field_names(self) -> List[str]:
#         marker = self._store.get(self._prefix)
#         return sorted(list(marker["children"])) if marker and self._is_object_marker(marker) else []
#
#     def get_fields(self) -> Dict[str, Any]:
#         result = {}
#         for name in self.get_field_names():
...
#             if self._is_object_marker(value):
#                 # 返回占位符 + 子字段名
#                 children = sorted(value.get("children")) if value.get("children") else []
#                 result[name] = f"__OBJ__:{children}"
#             else:
#                 result[name] = value
#         return result
...
#
#     def _is_object_marker(self, value:Any) -> bool:
#         return isinstance(value, dict) and value.get("__type__") == "__OBJ__"
